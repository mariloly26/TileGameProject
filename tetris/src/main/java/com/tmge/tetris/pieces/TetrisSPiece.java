...
public class TetrisSPiece extends TetrisPiece{
    ...
    protected boolean canRotate(int newRotationState) {
        switch (newRotationState){
            case 0:
                return isAvailableTile(pivotTile.right) && isAvailableTile(pivotTile.down) && isAvailableTile(pivotTile.down.left);
            case 1:
                return isAvailableTile(pivotTile.down) && isAvailableTile(pivotTile.left) && isAvailableTile(pivotTile.left.up);
            case 2:
                return isAvailableTile(pivotTile.left) && isAvailableTile(pivotTile.up) && isAvailableTile(pivotTile.up.right);
            case 3:
                return isAvailableTile(pivotTile.up) && isAvailableTile(pivotTile.right) && isAvailableTile(pivotTile.right.down);
            default:
                return false;
        }
    }
