...
public class TetrisLPiece extends TetrisPiece{
    @Override
    public void init(TetrisBufferBoard bufferBoard) {
        tiles = new TetrisTile[4];
        rotationState = 0;
        tiles[0] = bufferBoard.getBoard()[1][4]; //4
        tiles[1] = bufferBoard.getBoard()[2][4]; //3
        tiles[2] = bufferBoard.getBoard()[3][4]; //2
        tiles[3] = bufferBoard.getBoard()[3][5]; //1
        tiles[0].isInPiece = tiles[1].isInPiece = tiles[2].isInPiece = tiles[3].isInPiece = 1;
        bufferRowsCount = 3;
        pivotTile = tiles[1]; //update rotation pivot
        type = TetrisPieceType.L;
    }
    @Override
    ...
    protected boolean canRotate(int newRotationState){
        switch (newRotationState){
            case 0:
                return isAvailableTile(pivotTile.up) && isAvailableTile(pivotTile.down) && isAvailableTile(pivotTile.down.right);
            case 1:
                return isAvailableTile(pivotTile.right) && isAvailableTile(pivotTile.left) && isAvailableTile(pivotTile.left.down) ;
            case 2:
                return isAvailableTile(pivotTile.down) && isAvailableTile(pivotTile.up) && isAvailableTile(pivotTile.up.left);
            case 3:
                return isAvailableTile(pivotTile.left) && isAvailableTile(pivotTile.right) && isAvailableTile(pivotTile.right.up);
            default:
                return false;
        }
    }
